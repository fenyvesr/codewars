...
std::vector<int> allocateRooms(const std::vector<std::vector<int>>& customers) {
    auto mapping = ranges::views::iota(std::size_t{0}, customers.size())
                 | ranges::views::transform([&](auto i) { return std::tuple{customers[i][0], customers[i][1], static_cast<int>(i)}; })
    ...
    for (auto [arrive, leave, idx] : mapping)
        (rooms.empty() || rooms.top().first >= arrive)
            ? rooms.emplace(leave, allocation[idx] = rooms.size() + 1)
            : (allocation[idx] = rooms.top().second, rooms.pop(), rooms.emplace(leave, allocation[idx]));
    return allocation;
}
